...
\hypersetup{
  ...
  pdftitle={QANet Bug Repair and Mechanistic Analysis},
  pdfauthor={COMP4329 Group},
}
% ── TOC formatting ──
\renewcommand{\cfttoctitlefont}{\hfill\bfseries\fontsize{18}{22}\selectfont}
\renewcommand{\cftaftertoctitleskip}{0.5em}
\renewcommand{\cftaftertoctitle}{\hfill\null\par\vspace{0.5em}\begin{center}{\color{SectionRule}\rule{0.5\linewidth}{0.6pt}}\end{center}\vspace{0.6em}}
...
{\fontsize{11}{14}\selectfont\color{MetaGray}
The University of Sydney\\
School of Computer Science\\[0.8em]
COMP4329 Deep Learning --- 2026 Semester 1}

\vspace{2.5cm}

{\bfseries\fontsize{26}{32}\selectfont QANet Bug Analysis Report\par}

\vspace{0.8cm}

{\fontsize{14}{18}\selectfont
Comprehensive Debugging Analysis and\\
Mechanistic Investigation of a Repaired QANet Implementation\par}

\vspace{1.2cm}

{\color{SectionRule}\rule{0.6\linewidth}{0.8pt}\par}

\vspace{1.5cm}

{\fontsize{12}{16}\selectfont
Assignment 1 --- Group Report\par}

\vspace{\fill}

{\color{MetaGray}\fontsize{11}{14}\selectfont
\today\par}
...
\vspace*{1.5cm}
\begin{center}
{\bfseries\fontsize{18}{22}\selectfont Declaration of Authorship\par}
\vspace{0.5em}
{\color{SectionRule}\rule{0.5\linewidth}{0.6pt}\par}
\end{center}

\vspace{1.5cm}

\begin{center}
{\large\bfseries Haoguang Zhou}\par\smallskip
{\small SID 530029731 \quad\textbar\quad \href{mailto:hzho0918@uni.sydney.edu.au}{hzho0918@uni.sydney.edu.au}}

\vspace{0.8em}

{\large\bfseries Charles Wang}\par\smallskip
{\small SID 530096571 \quad\textbar\quad \href{mailto:ywan9676@uni.sydney.edu.au}{ywan9676@uni.sydney.edu.au}}

\vspace{0.8em}

{\large\bfseries Peiyao Shou}\par\smallskip
{\small SID 560243660 \quad\textbar\quad \href{mailto:psho0719@uni.sydney.edu.au}{psho0719@uni.sydney.edu.au}}
\end{center}

\vspace{1.0cm}

\begin{center}
{\small COMP4329 Deep Learning \quad\textbar\quad 2026 Semester 1\par
The University of Sydney}
\end{center}

\vspace{2.0cm}

\noindent We declare that this assignment is our own work. We have read and understood the University's policy on academic integrity and plagiarism. Where we have used external sources, they are properly cited. All experimental results reported in this document were produced by running the code ourselves.
...
\vspace*{0.5cm}
\begin{center}
{\bfseries\fontsize{18}{22}\selectfont Trustworthy AI Usage\par}
\vspace{0.5em}
{\color{SectionRule}\rule{0.5\linewidth}{0.6pt}\par}
\end{center}
...
\textbf{1.\quad Version control and traceability.}

\vspace{0.4em}

To ensure that the AI-assisted development process remains auditable and reproducible, every bug fix documented in this report was committed to the project's Git repository as one or more self-contained commits. Each commit is tagged with a repository-level identifier (e.g.\ \texttt{BUG-021}; closely related commits share a compound tag such as \texttt{BUG-012/013}), enabling reviewers to inspect, revert, or verify any individual fix in isolation. The complete commit history is publicly available on the \texttt{Fix-Stage-1} branch of the GitHub repository, providing an objective record of the incremental repair process and the human review that accompanied each change.

In the main text, bugs are referenced by a sequential reading number (BUG No.\,1, No.\,2, \ldots) for clarity. The underlying repository tags are non-sequential---reflecting merges, splits, and reorganisation during development---and are preserved in Appendix~B, which maps each sequential number to its original Git identifier.

\vspace{0.8em}

\textbf{2.\quad Methodological verification.}

\vspace{0.4em}

AI assistants can produce plausible-sounding but non-existent analytical techniques. To guard against such hallucinated methodology, every analysis method adopted in this report was traced back to its original publication and verified for suitability before use. All analytical methods cited in Sections~9--11 (e.g.\ causal tracing, linear probing, CKA, effective rank, selective corruption) correspond to peer-reviewed works listed in the bibliography. The AI was used to suggest candidate approaches and provide initial inspiration; the authors then independently located the source papers, confirmed that the method's assumptions and scope matched the experimental setting, and only then proceeded with implementation.

\vspace{0.8em}

\textbf{3.\quad Data integrity and analytical independence.}

\vspace{0.4em}

\begin{itemize}[leftmargin=1.6em]
\item \textbf{Cross-version reproducibility.}\quad Experimental code was developed iteratively with AI assistance. A numerical result was accepted only when independently generated code---produced by different AI agent sessions or manually revised versions---converged on the same values, and the underlying code passed manual inspection. Data that could not be reproduced under these conditions was discarded.
\item \textbf{Conservative interpretation.}\quad AI systems may amplify or exaggerate experimental findings to maintain narrative coherence. To mitigate this, all storylines, research questions, and experiment designs originated from the human authors. The AI served as an executor of implementation tasks, not as a source of scientific claims. For data interpretation, the authors first formulated conclusions independently; the AI was then prompted to \emph{challenge} those conclusions, and the analysis was iteratively refined through this adversarial process rather than by accepting AI-generated narratives at face value.
\item \textbf{Scope of AI contribution.}\quad The AI's roles were limited to: (i)~suggesting candidate bug fixes during debugging, (ii)~scaffolding experiment code (e.g.\ forward hooks, metric computation), and (iii)~polishing draft text. All polished text was reviewed and revised by the authors to ensure accuracy and to remove unsubstantiated claims.
\end{itemize}

\vspace{0.8em}

\textbf{4.\quad AI tool usage.}

\vspace{0.4em}

\textbf{Cursor IDE with Claude} was employed as the primary coding assistant. The following safeguards were maintained throughout:

\begin{itemize}[leftmargin=1.6em]
\item All code was \textbf{reviewed and validated by the author} before adoption. AI-generated suggestions were treated as proposals subject to human judgement, not as authoritative solutions.
\item All experimental results (F1/EM scores, AIE values, TE measurements, representation metrics) were produced by \textbf{executing code on Google Colab}. No numerical results were taken from AI output.
\item All analytical conclusions and scientific interpretations are the \textbf{author's own}.
\end{itemize}
...
% ──────────────────────────────────────────────────────────────────
% PREFACE / REPORT GUIDE
% ──────────────────────────────────────────────────────────────────
\thispagestyle{fancy}
\vspace*{0.5cm}
\begin{center}
{\bfseries\fontsize{18}{22}\selectfont Report Guide\par}
\vspace{0.5em}
{\color{SectionRule}\rule{0.5\linewidth}{0.6pt}\par}
\end{center}
...
\textbf{Report structure.}\quad This report comprises two parts. Sections~1--7 document the debugging and repair of a QANet (Yu et al.~\cite{yu2018qanet}) implementation for extractive question answering on a subset of SQuAD~1.1 (Rajpurkar et al.~\cite{rajpurkar2016squad}). Section~8 summarises the repaired system's performance and serves as a bridge to the mechanism study. Sections~9--11 present three hypothesis-driven mechanistic analyses of the repaired model: encoder convolution divergence (H1), CQ Attention directional asymmetry (H2), and Pointer layer wiring necessity (H3). Four appendices provide hyperparameter details (A), a bug-ID cross-reference index (B), supplementary data (C), and key code excerpts (D).

\vspace{0.6em}

\textbf{Experimental environment.}\quad All experiments were run on Google Colab (NVIDIA T4 / L4 GPU). The framework is PyTorch. Evaluation is performed on a subset of the SQuAD~1.1 development set. The trained checkpoint achieves F1\,=\,70.24, EM\,=\,59.22.

\vspace{0.6em}

\textbf{Notation and abbreviations.}
...
\midrule
$\Delta$F1 & Change in F1 score relative to baseline upon component removal (eval-time ablation) \\
AIE & Average Indirect Effect --- causal recovery of span probability when a single sub-layer is restored in a corrupted model \\
TE & Total Effect --- difference in span probability between clean and fully corrupted forward passes \\
JSD & Jensen--Shannon Divergence --- symmetric measure of attention weight distribution change \\
CKA & Centered Kernel Alignment --- similarity between two representation matrices \\
AUC & Area Under the ROC Curve --- classification accuracy of linear probes (answer-position detection) \\
Effective Rank & $\exp(\text{singular-value entropy})$ --- number of dimensions a representation effectively utilises \\
$\|W\|_F$ & Frobenius norm of a weight matrix --- proxy for learned dependency magnitude \\
CoV & Coefficient of Variation ($\sigma / \mu$) --- normalised dispersion of token norms \\
$M_1, M_2, M_3$ & Output of the 1st, 2nd, 3rd pass through the Model Encoder (Pointer layer inputs) \\
C2Q, Q2C & Context-to-Query and Query-to-Context attention directions within CQ Attention \\
\bottomrule
\end{tabularx}
\end{center}
...
\textbf{How to read this report.}\quad Readers interested only in the mechanistic findings may skip to Sections~9--11. Each hypothesis section is self-contained with its own research question, methodology, results, and conclusions. Debugging sections (1--7) provide the foundation: they establish that the codebase is correct and that the trained model is a legitimate experimental subject. Section~8 bridges the two parts by confirming the repaired model's performance.

\vspace{0.6em}

\textbf{Bug numbering convention.}\quad Each bug in Sections~1--7 is labelled with a sequential reading number (BUG No.\,1, No.\,2, \ldots, No.\,56). This numbering is assigned solely for reading convenience and does not reflect the chronological order of discovery or repair. The underlying Git repository uses a separate, non-sequential identifier system (e.g.\ \texttt{BUG-021}, \texttt{BUG-012/013}) that originated from the development workflow---merges, splits, and reorganisation during debugging produced gaps and out-of-order tags. Rather than renumber the commits and lose traceability, we retain the original repository tags and provide a cross-reference index in \textbf{Appendix~B}: each row maps a sequential reading number to its repository-level identifier and section location, allowing readers to locate the corresponding Git commit on the \texttt{Fix-Stage-1} branch.
...
This report documents the repair of a QANet implementation and the empirical evidence that the repaired system now behaves like a trainable extractive question-answering model rather than a collection of disconnected code paths. The analysis is organized around two intertwined goals. First, the end-to-end pipeline must become executable and empirically meaningful: input processing, forward propagation, loss computation, optimisation, validation, and checkpoint reload must all operate coherently. Second, the learning mechanisms themselves must match their intended theoretical roles, so that later mechanistic observations can be attributed to the model rather than to implementation artifacts.

\vspace{0.2em}

The later mechanism study addresses three hypotheses about how information is processed inside the repaired model. They focus on architectural behaviour rather than superficial hyperparameter comparison: what mechanistic differences cause architecturally identical encoder convolutions (Conv\_0 and Conv\_1) to diverge by an order of magnitude in causal importance ($\Delta$F1 ratio $\approx 30\times$) despite sharing the same architecture (H1); whether the directional asymmetry between C2Q and Q2C within CQ Attention, first observed in BiDAF, still holds after QANet's architectural change from LSTMs to convolutions and self-attention, and if so what mechanism maintains it (H2); and whether the Pointer layer's asymmetric wiring---$M_1$ as shared anchor, $M_2$ and $M_3$ as depth-specialised inputs---reflects genuine functional specialisation in the trained model, or whether the assumptions inherited from BiDAF break down under QANet's weight-shared iterative encoding (H3). The debugging sections below document these repairs and confirm that the resulting model is a correct QANet implementation, ensuring that findings in Sections~9--11 reflect genuine model behaviour rather than artifacts.
...
\textbf{Note on the Stage\,1\,/\,Stage\,2 classification.}\quad The original stage labels are retained but not used as an organisational principle, as the boundary is often ambiguous. Bugs are instead grouped by \textbf{functional module and category} (Section~1.2).
...
\toprule
\textbf{Section} & \textbf{Category} & \textbf{Count} & \textbf{Bug No.} \\
\midrule
\endfirsthead
\toprule
\textbf{Section} & \textbf{Category} & \textbf{Count} & \textbf{Bug No.} \\
...
\bottomrule
\endfoot
\textbf{\S 2} & Input Embedding \& Projection          & 5 & 1--5 \\
\midrule
\textbf{\S 3} & Convolution Operators                  & 3 & 6--8 \\
              & Encoder Architecture                   & 7 & 9--15 \\
              & Normalization, Activation \& Dropout   & 6 & 16--21 \\
              & Weight Initialization                  & 4 & 22--25 \\
\midrule
\textbf{\S 4} & Attention Mechanisms                   & 5 & 26--30 \\
\midrule
\textbf{\S 5} & \multicolumn{3}{l}{\textit{Structural repairs only; documented inline in Section 5.}} \\
\midrule
\textbf{\S 6} & Output Layer \& Loss Function          & 4 & 31--34 \\
\midrule
\textbf{\S 7} & Evaluation \& Inference                & 5 & 35--39 \\
              & Optimizers                             & 7 & 40--46 \\
              & Learning Rate Schedulers               & 5 & 47--51 \\
              & Training Pipeline                      & 5 & 52--56 \\
\end{longtable}
\normalsize
...
\begin{bugbox}{BUG No.\,1 \quad Context word/char embedding lookup swap}
...
\begin{bugbox}{BUG No.\,2 \quad Highway transpose fixed}
...
\begin{bugbox}{BUG No.\,3 \quad Character-tensor permutation before Conv2d fixed}
...
\begin{bugbox}{BUG No.\,4 \quad Pretrained word embeddings frozen}
...
\vspace{0.22em}
\FieldLabel{Impact on Training}
\textbf{Pre-fix:} Pre-trained GloVe representations drift during training, increasing overfitting risk on the relatively small SQuAD subset and weakening the intended lexical prior. Regularization is applied to the wrong quantity or with the wrong strength, so the model can overfit or suppress the wrong signal. \par\smallskip
...
\begin{bugbox}{BUG No.\,5 \quad Character embedding cross-word leakage removed}
...
\begin{bugbox}{BUG No.\,6 \quad Conv1d unfold dimension fixed}
...
\begin{bugbox}{BUG No.\,7 \quad Conv2d width-padding height mismatch fixed}
...
\begin{bugbox}{BUG No.\,8 \quad Depthwise-separable convolution order restored}
...
\begin{bugbox}{BUG No.\,9 \quad Positional-encoding frequency broadcasting fixed}
...
\begin{bugbox}{BUG No.\,10 \quad Encoder normalization indexing fixed}
...
\begin{bugbox}{BUG No.\,11 \quad Two-layer FFN restored}
...
\begin{bugbox}{BUG No.\,12 \quad Shared context/question encoder structure restored}
...
\begin{bugbox}{BUG No.\,13 \quad Embedding projection changed to standard Conv1d(k=1)}
...
\begin{bugbox}{BUG No.\,14 \quad Model-encoder input projection changed to standard Conv1d(k=1)}
...
\begin{bugbox}{BUG No.\,15 \quad Stochastic depth implemented}
...
\vspace{0.35em}
\FieldLabel{Corrective Action Taken}
Implement layer-level stochastic depth and apply it consistently across convolution, self-attention, and FFN sublayers with depth-aware survival probabilities. This supersedes the call-site portion of BUG No.\,12: the shared encoder route first had to be restored, and this change then redefined that shared route to carry the intended layer-dropout schedule.
\par
\vspace{0.35em}
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize,breaklines=true,breaksymbol={},frame=leftline,framerule=2pt,rulecolor=\color{gray!40},xleftmargin=6pt,formatcom=\singlespacing]
...
\begin{bugbox}{BUG No.\,16 \quad LayerNorm keepdim broadcasting fix}
...
\begin{bugbox}{BUG No.\,17 \quad LayerNorm affine order fixed}
...
\begin{bugbox}{BUG No.\,18 \quad GroupNorm grouping reshape order fixed}
...
\begin{bugbox}{BUG No.\,19 \quad ReLU clamp direction fixed}
...
\begin{bugbox}{BUG No.\,20 \quad LeakyReLU branch logic fixed}
...
\begin{bugbox}{BUG No.\,21 \quad Inverted dropout scaling fixed}
...
\begin{bugbox}{BUG No.\,22 \quad Kaiming normal formula fixed}
...
\begin{bugbox}{BUG No.\,23 \quad Kaiming uniform formula fixed}
...
\begin{bugbox}{BUG No.\,24 \quad Xavier normal denominator fixed}
...
\begin{bugbox}{BUG No.\,25 \quad Xavier uniform denominator fixed}
...
\begin{bugbox}{BUG No.\,26 \quad CQAttention batched-matmul order fixed}
...
\begin{bugbox}{BUG No.\,27 \quad Context/query masks passed in correct order}
...
\begin{bugbox}{BUG No.\,28 \quad Self-attention output no longer discarded}
...
\begin{bugbox}{BUG No.\,29 \quad Missing 1/√dk attention scaling restored}
...
\begin{bugbox}{BUG No.\,30 \quad Multi-head head/batch reassembly fixed}
...
\begin{bugbox}{BUG No.\,31 \quad NLL loss argument order fixed}
...
\begin{bugbox}{BUG No.\,32 \quad Pointer-head concatenation dimension fixed}
...
\begin{bugbox}{BUG No.\,33 \quad Extra 0.5 loss scaling removed}
...
\begin{bugbox}{BUG No.\,34 \quad Output/loss interface aligned around raw logits}
...
\vspace{0.35em}
\FieldLabel{Corrective Action Taken}
Make the pointer head return masked raw logits, move log\_softmax into the NLL loss, and apply log-prob conversion explicitly in evaluation decoding. This supersedes BUG No.\,33 inside the loss path: the earlier fix restored the correct loss magnitude, and this later interface repair restored the correct raw-logit versus log-probability contract.
\par
\vspace{0.35em}
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize,breaklines=true,breaksymbol={},frame=leftline,framerule=2pt,rulecolor=\color{gray!40},xleftmargin=6pt,formatcom=\singlespacing]
...
\begin{bugbox}{BUG No.\,35 \quad Checkpoint load key fixed to model\_state}
...
\begin{notebugbox}{BUG No.\,36 \quad torch.load(..., weights\_only=False) compatibility note}
...
\begin{bugbox}{BUG No.\,37 \quad Evaluation argmax dimension fixed}
...
\begin{bugbox}{BUG No.\,38 \quad Joint span decoding restored}
...
\vspace{0.35em}
\FieldLabel{Corrective Action Taken}
Replace independent argmax plus min/max swap with joint outer-product decoding under an upper-triangular validity mask. This supersedes BUG No.\,37: the earlier axis fix restored per-sample reduction, and this later repair then replaced the whole decoder with the correct joint span objective.
\par
\vspace{0.35em}
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize,breaklines=true,breaksymbol={},frame=leftline,framerule=2pt,rulecolor=\color{gray!40},xleftmargin=6pt,formatcom=\singlespacing]
\textcolor{gray}{# EvaluateTools/eval_utils.py L107-113}
...
\begin{rednotebugbox}{BUG No.\,39 \quad Checkpoint config is not restored during evaluation}
...
\begin{bugbox}{BUG No.\,40 \quad Adam state-key mismatch}
...
\begin{bugbox}{BUG No.\,41 \quad SGD-momentum state-key mismatch}
...
\begin{bugbox}{BUG No.\,42 \quad Adam weight-decay sign fixed}
...
\begin{bugbox}{BUG No.\,43 \quad Adam second moment uses squared gradients}
...
\begin{bugbox}{BUG No.\,44 \quad Adam bias-correction exponentiation fixed}
...
\begin{bugbox}{BUG No.\,45 \quad SGD weight-decay sign fixed}
...
\begin{bugbox}{BUG No.\,46 \quad SGD momentum update direction fixed}
...
\begin{bugbox}{BUG No.\,47 \quad Cosine scheduler math.pi fix}
...
\begin{bugbox}{BUG No.\,48 \quad Missing 0.5 factor in cosine scheduler fixed}
{\color{MetaGray}\small Stage: 2 \;\textbar\; Category: Deep Learning Mechanism / Learning Rate Scheduling \;\textbar\; Source: Schedulers/cosine\_scheduler.py L28\par}
\vspace{0.22em}
\FieldLabel{Nature}
...
\begin{bugbox}{BUG No.\,49 \quad Lambda scheduler multiplicative fix}
...
\begin{bugbox}{BUG No.\,50 \quad Step scheduler exponentiation fix}
...
\begin{bugbox}{BUG No.\,51 \quad Lambda warmup scheduler redesigned into a picklable object}
...
\begin{bugbox}{BUG No.\,52 \quad argparse.Namespace construction fixed}
...
\begin{bugbox}{BUG No.\,53 \quad loss.item().backward() fixed to loss.backward()}
...
\begin{bugbox}{BUG No.\,54 \quad Gradient clipping moved before optimizer step}
...
\begin{bugbox}{BUG No.\,55 \quad Best checkpoint retained explicitly}
...
\begin{bugbox}{BUG No.\,56 \quad EMA support added and integrated}
...
\begin{center}
\SafeIncludeGraphics[width=0.65\linewidth]{prism-uploads/adam_lambda_loss_shared_step_40800_exact_style.png}
...
The QANet paper's ablation study (Table~5, \cite{yu2018qanet}) shows that convolution contributes more to performance than Self-Attention ($-2.7$~F1 vs $-1.3$~F1 upon removal, measured after retraining). However, the paper treats both convolution layers as a single unit and does not examine Conv\_0 and Conv\_1 separately.
...
Conv\_1 is the \textbf{most important} of all four component types ($-32.54$~F1 upon removal), while Conv\_0 is the \textbf{least important} ($-1.09$~F1)---a $\Delta$F1 ratio of $\approx 30\times$.\footnote{The precise multiplier is sensitive to the small denominator ($-1.09$~F1) and varies by method: causal tracing gives $\approx 4.7\times$ (AIE share). The \emph{qualitative ordering} (Conv\_1 $\gg$ Conv\_0) is robust across both methods; ``$30\times$'' is retained as shorthand for the order-of-magnitude gap.} Yet Conv\_0 and Conv\_1 are \textbf{architecturally identical}: both are depthwise separable convolutions with kernel size $k=5$ and the same hidden dimension, trained end-to-end under the same objective. The same architecture produces both the most important and the least important component---architecture alone cannot explain this gap; the divergence must relate to their different positions in the pipeline and the different functions they consequently learn during training.
...
All ablations are performed at eval-time (zeroing out component outputs without retraining), following the occlusion-based methodology introduced by Zeiler \& Fergus~\cite{zeiler2014visualizing} and formalised by Meyes et al.~\cite{meyes2019ablation}. Ideally, retraining after ablation would control for compensatory adaptation, but each retraining run requires a full GPU training cycle, infeasible under Colab quota constraints. Since the core objective is to determine the \textbf{relative importance ordering} among components (rather than precise absolute differences), eval-time ablation is sufficient for qualitative conclusions. The QANet paper's retrain-based ablation (Yu et al.~\cite{yu2018qanet}) also ranks convolution above Self-Attention, consistent with our ordering.

\textbf{Training-time cross-check.}\quad As an independent sanity check, we conducted separate retraining experiments in which either all encoder convolutions or all encoder self-attention layers were removed and the model was retrained from scratch. Removing convolution yielded F1\,=\,59.51 ($\Delta$\,=\,$-10.73$), while removing self-attention yielded F1\,=\,66.39 ($\Delta$\,=\,$-3.85$). The conv/attn importance ratio under training-time ablation is $2.8\times$, compared with $2.1\times$ in the original paper and $9.0\times$ in our eval-time ablation. The qualitative ordering \textbf{conv $>$ attention} is consistent across all three methods; the eval-time ratio is larger because distribution mismatch disproportionately amplifies the impact of removing the dominant component. This confirms that our eval-time ordering reflects genuine functional importance. An additional experiment replacing depthwise separable convolution with standard convolution (retrained) yielded F1\,=\,67.54 ($\Delta$\,=\,$-2.70$), indicating that the separable factorisation contributes meaningfully but is not the primary driver of convolution's importance.
...
Zero out all 21 instances of a component type (eval-time, full dev set). Zero-out = sub-layer output replaced with zeros before residual addition.
...
Remove conv (both) or self\_attn in one (pass, block) at a time: $3 \text{ passes} \times 7 \text{ blocks} \times 2 \text{ component types} = 42$ configs, full dev set.
...
\FieldLabel{9.3.3 Causal tracing (Meng et al.~\cite{meng2022locating})}

Meng et al.\ developed causal tracing to localise factual associations in GPT via a corruption--restoration protocol. We adapt this protocol to QANet's encoder to identify which sub-layers carry the causal information for answer prediction.
...
Since linear probes show Conv\_0 and Conv\_1 encode similar information, we turn to \emph{how} that information is structured. Ethayarajh~\cite{ethayarajh2019contextual} showed that geometric properties such as anisotropy and self-similarity vary substantially across layers of BERT, ELMo, and GPT-2, motivating representational structure as a complementary lens to information content. We compare Conv\_1 vs Conv\_0 output properties directly. 200 samples, 5 (pass, block) positions. Metrics: local contrast ($1 - \text{lag-1 cosine similarity}$); token norm variance (CoV); and effective rank ($\exp$ of singular value entropy, Roy \& Vetterli~\cite{roy2007effective}).
...
The divergence across metrics has diagnostic value. Conv\_0 scores slightly higher on directional distinctiveness yet is nearly dispensable ($-1.09$~F1)---ruling out directional distinctiveness as a sufficient condition. The two dimensions where Conv\_1 holds a clear advantage---amplitude modulation and subspace dimensionality---are candidate mechanisms. This is consistent with the scaled dot-product attention mechanism (Vaswani et al.~\cite{vaswani2017attention}): $QK^{\!\top}$ scores are sensitive to input magnitude differences, and higher-dimensional inputs allow $Q$, $K$ projections to carry richer discriminative signals. Notably, FFN provides a natural control: its Norm Variance (14.79) is nearly identical to Conv\_1's (14.48), so if Norm Variance alone were sufficient for importance, FFN should match Conv\_1 in ablation impact. It does not ($-3.32$ vs $-32.54$~F1), precisely because FFN sits \emph{downstream} of Self-Attention---its output properties cannot shape $QK^{\!\top}$ scores. This confirms that amplitude modulation's causal impact depends critically on pipeline position: the same output property matters when it feeds into attention, and is irrelevant when it does not.
...
Self-Attention is Conv\_1's direct downstream consumer. Clark et al.~\cite{clark2019bert} showed that JS divergence (Lin~\cite{lin1991divergence}) and attention entropy are effective lenses for characterising how transformer attention heads behave---distinguishing broad-attending from focused heads and quantifying distributional shifts. We adopt the same metrics to measure how attention distributions change under ablation. We use forward hooks to capture attention weights under three conditions (50 samples; sufficient because the per-block JSD ratio is tightly concentrated at 7.0--10.3$\times$ across all 7 blocks, indicating low variance): clean, $-$Conv\_1, $-$Conv\_0 (control).
...
\textbf{Finding~9}: Attention distortion under Conv\_1 removal is \textbf{8.2$\times$ greater} than under Conv\_0 removal. The ratio is consistent across all 7 blocks (7.0--10.3$\times$).
...
QANet (Yu et al.~\cite{yu2018qanet}) inherits its bidirectional CQ Attention design from BiDAF (Seo et al.~\cite{seo2017bidaf}). This layer fuses the independently encoded Context and Question streams into a single representation. The output is a concatenation of four sub-components:
...
Seo et al.~\cite{seo2017bidaf} (Table~1b) measured the relative importance of the two directions via \emph{training-time} ablation (removing a component and retraining the model):

\small
\begin{center}
{\bfseries Table 10.1. BiDAF direction-level ablation (Seo et al.~\cite{seo2017bidaf})}\\[0.25em]
...
We choose \textbf{eval-time ablation}---zeroing sub-component outputs directly on a fully trained model. This is a deliberate methodological choice, not a fallback due to resource constraints. Our research goal is \textbf{mechanistic analysis}---understanding the functional roles that components \emph{currently serve} in the trained model---not architecture search. This aligns with the causal tracing methodology of Meng et al.~\cite{meng2022locating} for localising factual associations in GPT: intervening on a trained model (corruption + restoration) to localise functional roles. Training-time ablation allows the model to learn compensatory strategies during retraining, erasing the functional traces of the removed component---making it \emph{unsuitable} for mechanistic analysis.
...
\texttt{cq\_resizer} is \texttt{Conv1d(4d, d, kernel\_size=1)}, with weight $W$ of shape $[d, 4d]$. We partition $W$ by columns into four $[d, d]$ quadrants and compute each quadrant's Frobenius norm as a proxy for learned dependency, following the magnitude-based importance framework of Li et al.~\cite{li2017pruning} and Han et al.~\cite{han2015learning}. Ablation measures \emph{performance loss} from removing a component (output side); weight norm measures the model's \emph{learned dependency} (parameter side). If ablation ranking and weight norm ranking agree, conclusion credibility is significantly strengthened.
...
We test the information concentration premise through selective corruption---injecting targeted noise into specific input positions to measure their causal contribution, following the intervention-based interpretability paradigm (Vig et al.~\cite{vig2020investigating}; Finlayson et al.~\cite{finlayson2021causal}).
...
QANet's (Yu et al.~\cite{yu2018qanet}) Model Encoder applies the same 7 encoder blocks iteratively three times (weight-shared), producing three intermediate representations:
...
We directly test the necessity of the asymmetric design through wiring replacement. At eval time, we change the Pointer's input combinations (encoder unchanged), evaluating on the full dev set. The sample size ensures that even small F1 differences (e.g., 0.23) are measured with high precision.
...
To measure how much representational structure is shared between iteration products, we use CKA (Centered Kernel Alignment; Kornblith et al.~\cite{kornblith2019similarity}). CKA was specifically designed to compare neural network representations across layers or models---it is invariant to orthogonal transformations and isotropic scaling, making it robust to the kind of representational format changes that occur across shared-weight iterations. We compute linear CKA for $M_1$--$M_2$, $M_2$--$M_3$, $M_1$--$M_3$ (full dev set).
...
\textbf{Method}: Following the linear probing approach (Alain \& Bengio~\cite{alain2016understanding}; Hewitt \& Manning~\cite{hewitt2019structural}), train logistic regression on $M_1$/$M_2$/$M_3$'s per-token representations for \texttt{is\_start\_token} and \texttt{is\_end\_token}. 500 samples, 80/20 split, ROC-AUC. The Pointer itself is a linear projection ($w \cdot [M;\, M']$), so linear probes test the \emph{same type} of information readability.
...
Each encoder block's pipeline is Conv\_0 $\to$ Conv\_1 $\to$ Self-Attention $\to$ FFN. Self-Attention (Vaswani et al.~\cite{vaswani2017attention}) is the only global operation. Li et al.~\cite{li2018deeper} first identified that iteratively applying message-passing operators in GCNs causes token representations to converge toward the global mean (``over-smoothing''); Dong et al.~\cite{dong2021attention} proved that pure self-attention suffers doubly-exponential rank collapse with depth. QANet's weight-shared iteration applies the \emph{same} self-attention operator three times---a setting that closely mirrors these theoretical concerns. If the third iteration's Self-Attention aggregates representations toward the global mean, adjacent tokens become more similar, making precise boundary positions harder to distinguish.

\textbf{Method}: To detect over-smoothing, we need metrics that capture local discriminability between adjacent token representations. Ethayarajh~\cite{ethayarajh2019contextual} showed that such geometric properties (self-similarity, anisotropy) change systematically across layers of BERT and GPT-2; Kriegeskorte et al.~\cite{kriegeskorte2008representational} provided the general representational similarity framework. We adapt these approaches to measure the $M_1 \to M_2 \to M_3$ trajectory. Compute the following metrics on $M_1$/$M_2$/$M_3$ (200 samples):
...
\ReportSection{Appendix A\quad Experimental Hyperparameters}
...
\midrule
Batch size & 32 \\
Learning rate & $1 \times 10^{-3}$ (Adam, warmup 1000 steps) \\
Hidden dimension ($d_{\text{model}}$) & 96 \\
Encoder blocks & 7 per pass \\
Model Encoder passes & 3 ($\to M_1, M_2, M_3$) \\
Attention heads & 1 \\
Convolution kernel size & 5 (depthwise separable) \\
Dropout & 0.1 \\
Stochastic depth ($p_L$) & 0.1 \\
Checkpoint & Step 40,800 (early stopping) \\
Evaluation set & SQuAD 1.1 subset (Rajpurkar et al.~\cite{rajpurkar2016squad}) dev split \\
Causal tracing samples & 200 (H1, H2), 50 (attention analysis) \\
Noise scale ($\sigma$) & $3 \times \text{std}(\text{clean activations})$ \\
\bottomrule
\end{tabularx}
\end{center}

\ReportSection{Appendix B\quad Bug Cross-Reference Index}

The bug identifiers used throughout this report (e.g.\ \texttt{BUG-021}, \texttt{BUG-N013}) originate from the Git commit-level indexing system maintained in the project repository. Non-sequential numbering reflects merges, splits, and reorganisation that occurred during the development cycle; each identifier maps one-to-one to a traceable commit on the \texttt{Fix-Stage-1} branch. The table below provides a sequential reading index for convenience.
...
\begin{longtable}{
  ...
  >{\ttfamily\raggedright\arraybackslash}p{0.24\linewidth}
  >{\raggedright\arraybackslash}p{0.16\linewidth}
  >{\raggedright\arraybackslash}p{0.44\linewidth}}
\toprule
\textbf{No.} & \textbf{Report ID} & \textbf{Section} & \textbf{Brief description}\\
\midrule
\endfirsthead
\toprule
\textbf{No.} & \textbf{Report ID} & \textbf{Section} & \textbf{Brief description}\\
...
\bottomrule
\endfoot
% ── Section 2: Input Embedding & Projection ──
1  & BUG-021       & \S 2 & Context word/char embedding lookup swap\\
2  & BUG-012/013   & \S 2 & Highway layer transpose error\\
3  & BUG-014/015   & \S 2 & Char tensor permutation before Conv2d\\
4  & BUG-064       & \S 2 & Pre-trained word vectors not frozen\\
5  & BUG-N013      & \S 2 & Char embedding cross-word leakage\\
\midrule
% ── Section 3: Convolution Operators ──
6  & BUG-006       & \S 3 & Conv1d / unfold dimension mismatch\\
7  & BUG-007/008   & \S 3 & Conv2d width padding / height mismatch\\
8  & BUG-009/010/011 & \S 3 & Depthwise-separable conv op order\\
\midrule
% ── Section 3: Encoder Architecture ──
9  & BUG-016       & \S 3 & Positional encoding frequency broadcast\\
10 & BUG-017/018/019 & \S 3 & Encoder normalisation index error\\
11 & BUG-N006      & \S 3 & Two-layer FFN structure restored\\
12 & BUG-N008      & \S 3 & Shared context/question encoder restored\\
13 & BUG-N009      & \S 3 & Embedding projection to Conv1d(k=1)\\
14 & BUG-N010      & \S 3 & Model-encoder input projection to Conv1d(k=1)\\
15 & BUG-N012      & \S 3 & Stochastic depth implemented\\
\midrule
% ── Section 3: Normalisation, Activation & Dropout ──
16 & BUG-004       & \S 3 & LayerNorm keepdim broadcast\\
17 & BUG-044       & \S 3 & LayerNorm affine order error\\
18 & BUG-037       & \S 3 & GroupNorm reshape order\\
19 & BUG-036       & \S 3 & ReLU clamp direction\\
20 & BUG-035       & \S 3 & LeakyReLU branch logic\\
21 & BUG-048       & \S 3 & Dropout inverted scaling\\
\midrule
% ── Section 3: Weight Initialisation ──
22 & BUG-041       & \S 3 & Kaiming normal formula\\
23 & BUG-042       & \S 3 & Kaiming uniform formula\\
24 & BUG-043       & \S 3 & Xavier normal denominator\\
25 & BUG-062       & \S 3 & Xavier uniform denominator\\
\midrule
% ── Section 4: Attention Mechanisms ──
26 & BUG-005       & \S 4 & CQAttention batch matmul order\\
27 & BUG-051       & \S 4 & Context/query mask argument swap\\
28 & BUG-038/050   & \S 4 & Self-attention output discarded\\
29 & BUG-N003      & \S 4 & Missing $1/\sqrt{d_k}$ attention scaling\\
30 & BUG-049       & \S 4 & Multi-head attention head/batch reshape\\
\midrule
% ── Section 6: Output Layer & Loss Function ──
31 & BUG-003       & \S 6 & NLL loss argument order\\
32 & BUG-020       & \S 6 & Pointer head concatenation dimension\\
33 & BUG-N007      & \S 6 & Spurious 0.5 loss scaling\\
34 & BUG-N016      & \S 6 & Output/loss interface unified to raw logits\\
\midrule
% ── Section 7: Evaluation & Inference ──
35 & BUG-001/002   & \S 7 & Checkpoint load key mismatch\\
36 & BUG-N002      & \S 7 & torch.load weights\_only compatibility\\
37 & BUG-034       & \S 7 & Eval argmax dimension error\\
38 & BUG-N011      & \S 7 & Joint span decoding restored\\
39 & BUG-N018      & \S 7 & Eval checkpoint config not restored\\
\midrule
% ── Section 7: Optimizers ──
40 & BUG-022       & \S 7 & Adam optimizer state key mismatch\\
41 & BUG-023       & \S 7 & SGD with momentum state key mismatch\\
42 & BUG-052       & \S 7 & Adam weight decay sign error\\
43 & BUG-053       & \S 7 & Adam second moment: gradient squared\\
44 & BUG-039/040   & \S 7 & Adam bias correction exponent error\\
45 & BUG-054       & \S 7 & SGD weight decay sign error\\
46 & BUG-055       & \S 7 & SGD momentum update direction\\
\midrule
% ── Section 7: Learning Rate Schedulers ──
47 & BUG-024       & \S 7 & Cosine scheduler math.pi error\\
48 & BUG-056       & \S 7 & Cosine scheduler missing 0.5 factor\\
49 & BUG-057       & \S 7 & Lambda scheduler multiplication form\\
50 & BUG-058       & \S 7 & Step scheduler exponent error\\
51 & BUG-N001      & \S 7 & Lambda warmup scheduler picklability\\
\midrule
% ── Section 7: Training Pipeline ──
52 & BUG-025       & \S 7 & argparse.Namespace construction error\\
53 & BUG-026       & \S 7 & loss.item().backward() call error\\
54 & BUG-059       & \S 7 & Gradient clipping before optimizer.step\\
55 & BUG-N015      & \S 7 & Explicit best-checkpoint retention\\
56 & BUG-N017      & \S 7 & EMA integration\\
\end{longtable}
\normalsize

\ReportSection{Appendix C\quad Supplementary Experimental Data}

\textit{Reserved for supplementary tables, additional ablation configurations, or extended result breakdowns not included in the main text.}

\ReportSection{Appendix D\quad Key Code Excerpts}

\textit{Reserved for annotated excerpts of critical implementation code (e.g., the causal tracing forward hook, selective corruption masking, representation structure metrics).}
...
% ── Footnote: remove the default separator bar ──
\renewcommand{\footnoterule}{\vspace{0.4em}}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small COMP5329 / COMP4329\enspace Deep Learning}
\fancyhead[R]{\small The University of Sydney}
\fancyfoot[C]{\small Due: Monday, 13 April 2026, 23:59 (Sydney Time)
              \quad|\quad Page~\thepage}
...
\begin{center}
    {\normalsize COMP5329 / COMP4329} \\[0.4em]
    {\Huge\bfseries \textbf{Deep Learning}}    \\[1.2em]
    \rule{\linewidth}{0.8pt}          \\[1.0em]
    {\Large\bfseries Assignment 1: Debugging and Experiments} \\[0.6em]
    {\large\itshape QANet Bug Repair and Mechanistic Analysis}
\end{center}
...
\begin{tabular}{C{0.8cm} | L{6.2cm} | C{4.0cm} | C{3.2cm}}
    \toprule
    \textbf{\#} & \textbf{Full Name} & \textbf{Student ID} & \textbf{UniKey} \\
    \midrule
    1 & Haoguang Zhou & 530029731 & hzho0918 \\[1.4em]
    \midrule
    2 & Charles Wang  & 530096571 & ywan9676 \\[1.4em]
    \midrule
    3 & Peiyao Shou   & 560243660 & psho0719 \\[0.2em]
    \bottomrule
\end{tabular}
...
\begin{tabular}{L{3.8cm} | L{11.2cm}}
    \toprule
    \textbf{GitHub}            & \url{https://github.com/1355-XCZ/Assignment1_2026} \\
    \midrule
    \textbf{Google Drive Link} & \url{https://drive.google.com/drive/folders/1eOBr8ZURxk3jv8Tn1UxyPXCV26MFCaMY} \\
    \midrule
    \textbf{Submission Date}   & 13 April 2026 \\
    \bottomrule
\end{tabular}
...
\begin{center}
    \small School of Computer Science \,\textperiodcentered\, Faculty of Engineering \,\textperiodcentered\, The University of Sydney
\end{center}
...
\textbf{Report structure.}\quad This report comprises two parts. Sections~1--7 document the debugging and repair of a QANet (Yu et al.~\cite{yu2018qanet}) implementation for extractive question answering on a subset of SQuAD~1.1 (Rajpurkar et al.~\cite{rajpurkar2016squad}). Section~8 confirms the repaired model's baseline performance. Sections~9--11 present three hypothesis-driven mechanistic analyses of the repaired model: encoder convolution divergence (H1), CQ Attention directional asymmetry (H2), and Pointer layer wiring necessity (H3). Three appendices provide hyperparameter details (A), a bug-ID cross-reference index (B), and a software/foundational-methods summary (C).
...
\textbf{How to read this report.}\quad Readers interested only in the mechanistic findings may skip to Sections~9--11. Each hypothesis section is self-contained with its own research question, methodology, results, and conclusions. Debugging sections (1--7) provide the foundation: they establish that the codebase is correct and that the trained model is a legitimate experimental subject. Section~8 confirms the repaired model's baseline performance.
...
This report documents the repair of a QANet implementation and the empirical evidence that the repaired system now behaves like a trainable extractive question-answering model rather than a collection of disconnected code paths. The analysis is organized around two successive goals. First, the end-to-end pipeline must become executable and empirically meaningful: input processing, forward propagation, loss computation, optimisation, validation, and checkpoint reload must all operate coherently. Second, the learning mechanisms themselves must match their intended theoretical roles, so that later mechanistic observations can be attributed to the model rather than to implementation artifacts.
...
\ReportSection{8 Notebook Implementations and Results}
...
The same 7 encoder blocks are applied iteratively three times (weight-shared), producing intermediate representations $M_1$, $M_2$, $M_3$ after each pass.
...
\textbf{Experiment~1 (Is the gap real?)} We quantify component importance using two independent methods---eval-time ablation (global and per-block) and causal tracing---and check for output-magnitude confounds. The goal is to establish that the Conv\_1\,$\gg$\,Conv\_0 importance gap is reproducible and not an artefact.

\textbf{Experiment~2 (What is the functional difference?)} Experiment~1 confirms that Conv\_1 is far more important than Conv\_0 ($\Delta$F1 ratio $\approx 30\times$). So how do their outputs differ functionally? We first test with linear probes and find that both encode nearly identical task-relevant information (AUC gap only 0.025)---information difference cannot explain an order-of-magnitude importance gap. We therefore examine whether the two layers apply different transformations to their inputs via representational structure metrics and spatial coherence analysis.

\textbf{Experiment~3 (How does the model fail?)} Experiment~2 characterises the output differences. What happens inside the model when Conv\_1 is removed? Since Self-Attention is Conv\_1's direct downstream consumer, we capture attention weights and measure how attention distributions (JS divergence, entropy, answer focus) change under Conv\_1 removal.

\ReportSubsection{9.3 Experiment 1: Establishing the gap}

\FieldLabel{9.3.1 Methodological note: eval-time ablation}

All ablations are performed at eval-time (zeroing out component outputs without retraining), following the occlusion-based methodology introduced by Zeiler \& Fergus~\cite{zeiler2014visualizing} and formalised by Meyes et al.~\cite{meyes2019ablation}. The core objective is to determine the \textbf{relative importance ordering} among components; eval-time ablation is sufficient for this qualitative ranking.\footnote{Retraining after ablation would control for compensatory adaptation but is infeasible under Colab quota constraints. Zero-out is preferred over mean or noise replacement under residual architectures: the latter inject actively incorrect signals that propagate forward and confound interpretation, whereas zero-out asks the narrower question of what remains once the model is denied a component's contribution.} As a cross-check, we conducted separate retraining experiments removing all convolutions or all self-attention layers; the qualitative ordering \textbf{conv\,$>$\,attention} held across our eval-time ablation ($9.0\times$), our retrain ablation ($2.8\times$), and the original paper ($2.1\times$).\footnote{Retrain results: $-$conv F1\,=\,59.51 ($\Delta$\,=\,$-10.73$); $-$attn F1\,=\,66.39 ($\Delta$\,=\,$-3.85$). The eval-time ratio is larger because distribution mismatch disproportionately amplifies the dominant component's removal. An additional experiment replacing depthwise separable with standard convolution (retrained, F1\,=\,67.54, $\Delta$\,=\,$-2.70$) confirms the separable factorisation contributes but is not the primary driver. These retraining experiments were conducted as standalone verification runs and are not included in the main experimental pipeline of this report.}

\FieldLabel{9.3.2 Global ablation}

Zero out all $3 \times 7 = 21$ instances of a component type (eval-time, full dev set). Zero-out = sub-layer output replaced with zeros before residual addition.\footnote{$\Delta$F1 values are computed from full-precision scores before rounding; minor discrepancies with the rounded F1 column are due to independent rounding.}
...
The table reveals a stark asymmetry: Conv\_1 removal causes the largest drop among all four component types ($-32.54$~F1), whereas Conv\_0 removal causes the smallest ($-1.09$~F1)---a ratio of $\approx 30\times$ between architecturally identical layers (see note on ratio sensitivity in Section~9.1).

Moreover, joint removal of both convolutions ($-43.11$) exceeds the sum of their individual effects ($-33.63$) by 9.48~F1---a super-additive interaction. Conv\_0's individual deficit is small ($-1.09$), yet its absence materially worsens the outcome when Conv\_1 is also removed, suggesting the two layers play complementary rather than redundant roles. Experiment~2 examines how the two layers' outputs differ functionally.

\FieldLabel{9.3.3 Per-block ablation}

To test whether importance is concentrated in specific blocks or distributed across the encoder, we zero out conv (Conv\_0 and Conv\_1 jointly) or self-attention at a single (pass, block) location at a time, yielding $3 \text{ passes} \times 7 \text{ blocks} \times 2 \text{ component groups} = 42$ configurations, each evaluated on the full dev set.

The largest single-block drop is only $-1.23$~F1 (pass~0, block~0, conv), whereas collective removal causes $-43.11$~F1---a $35\times$ gap. No individual block is essential, yet the ensemble is indispensable: importance is distributed redundantly across blocks.

Averaging per-block $\Delta$F1 within each pass reveals a gradient: $M_1$\,=\,$-0.16$, $M_2$\,=\,$-0.06$, $M_3$\,=\,$+0.01$---earlier passes carry more marginal importance, consistent with foundational representations being established early and later passes performing incremental refinement.

\FieldLabel{9.3.4 Causal tracing (Meng et al.~\cite{meng2022locating})}
...
Causal tracing independently confirms the importance gap: Conv\_1 accounts for 71.3\% of total AIE. Conv\_0 ranks second (15.3\%), above Attn and FFN, indicating it does carry causal weight even though ablation damage is minimal. This asymmetry---small ablation damage yet moderate causal recovery---is consistent with Conv\_0 providing a contribution that, while useful, is compensable via the residual pathway.

\begin{center}
\SafeIncludeGraphics[width=0.92\linewidth]{prism-uploads/h1_fig1_causal_tracing_heatmap.png}\\[0.3em]
{\small\textbf{Figure 9.1.} Causal tracing heatmap: AIE per sub-layer across 3 passes $\times$ 7 blocks. Conv\_1 (top-right) dominates all other panels under a shared colour scale. Early passes contribute more AIE than later passes, consistent with the early-pass dominance observed in per-block ablation.}
\end{center}

\FieldLabel{9.3.5 Confound check: output magnitude}

Sub-layer L2 norms: Conv\_1 (250) $>$ Conv\_0 (113) $>$ FFN (104) $>$ Attn (82). Pearson $r(\text{norm}, \text{AIE}) = 0.893$.

Could the gap simply reflect Conv\_1 having larger outputs? If magnitude were the driver, the norm ordering Conv\_0 (113) $>$ FFN (104) $>$ Attn (82) should predict ablation ordering Conv\_0 $>$ FFN $>$ Attn. The actual ablation ordering is exactly reversed: Attn ($-4.81$) $>$ FFN ($-3.32$) $>$ Conv\_0 ($-1.09$)---\textbf{inversely} correlated. This rules out output magnitude as the driver for these three components. Conv\_1's dominance direction is robust regardless of magnitude; however, because Conv\_1 also has the largest norm, the precise magnitude of its advantage over other components carries uncertainty from norm co-correlation.

\FieldLabel{9.3.6 Experiment 1 summary}

The order-of-magnitude importance gap between identical architectures is real and reproducible across two independent methods. It cannot be explained by output magnitude alone. What Conv\_1 \emph{learns to do} must differ qualitatively from Conv\_0.
...
Linear probing is a standard technique in representation analysis, originally proposed by Alain \& Bengio~\cite{alain2016understanding} and extended to NLP by Belinkov et al.~\cite{belinkov2017neural}. A linear probe measures what information is \emph{linearly accessible} in a representation---precisely the kind of information a downstream linear layer (such as QANet's Pointer) can exploit. We train logistic regression on per-token sub-layer outputs \textbf{before residual addition}---i.e., the sub-layer's own transformation, isolated from the skip-connection input.\footnote{Probing the post-residual stream would mix the sub-layer's contribution with accumulated upstream information, making all components appear similar regardless of their actual transformation.} Label: inside answer span. 500 samples, 80/20 split, 5 (pass, block) positions.
...
On the answer-position property tested, all components encode similar information even in their isolated pre-residual transformations. The Conv\_1 vs Conv\_0 gap is only 0.025~AUC, negligible compared to their order-of-magnitude ablation gap.

\textbf{Limitation}: We only probed one property (answer position); Conv\_1 and Conv\_0 may differ on untested properties (e.g., syntactic boundaries, entity types), so the ``similar information content'' conclusion is strictly limited to this property---\textbf{the gap's root cause may partly lie in untested information differences, partly in representational structure}. Additionally, linear probes capture only linearly separable features.

\FieldLabel{9.4.2 Representational structure (discriminativity)}

Since linear probes show Conv\_0 and Conv\_1 encode similar information, we turn to \emph{how} that information is structured. Ethayarajh~\cite{ethayarajh2019contextual} showed that geometric properties such as anisotropy and self-similarity vary substantially across layers of BERT, ELMo, and GPT-2, motivating representational structure as a complementary lens to information content. We compare Conv\_1 vs Conv\_0 output properties directly. 200 samples, 5 (pass, block) positions. Metrics: local contrast ($1 - \text{lag-1 cosine similarity}$); token norm variance and its coefficient of variation (Norm CoV); and effective rank ($\exp$ of singular value entropy, Roy \& Vetterli~\cite{roy2007effective}).
...
Conv\_1 exceeds Conv\_0 on two dimensions: token-level amplitude variation (Norm Variance 3.2$\times$) and output subspace dimensionality (Effective Rank 1.3$\times$). However, in directional distinctiveness (Local Contrast) and normalised variation (Norm CoV), Conv\_0 is actually slightly higher. Conv\_1's transformation is thus primarily characterised by \textbf{amplitude modulation and subspace dimensionality}, rather than directional separation of adjacent tokens. Conv\_0's outputs, while more directionally distinctive, have minimal overall impact (ablation only $-1.09$~F1), ruling out directional distinctiveness as a sufficient condition for importance.

\FieldLabel{9.4.3 Spatial structure (local coherence)}

Representational structure metrics characterise each sub-layer's output in isolation; we complement them with a spatial lens. Ethayarajh~\cite{ethayarajh2019contextual} measured self-similarity \emph{across layers}; here we apply the same cosine-similarity tool \emph{across token positions} to capture how each sub-layer's output varies with positional distance. Mean cosine similarity between output vectors at positions $t$ and $t+k$, measured at pass~0, block~0; 500 samples.
...
Conv\_0's decay (0.109) is nearly double Conv\_1's (0.058), indicating Conv\_0's output has stronger locality. Conv\_1's flatter decay means its output is more spatially uniform.\footnote{Conv\_1's intermediate lags (lag=3, 5) are slightly higher than lag=2, producing a non-monotonic profile. The endpoint decay (lag~1$\to$5) nonetheless captures the overall trend; more positions and samples would be needed to determine whether the non-monotonicity reflects a genuine spatial pattern or sampling variability at a single (pass, block) location.} Self-Attention has the highest overall similarity (0.84) with near-zero decay (0.037), consistent with its global mixing role.

\FieldLabel{9.4.4 Experiment 2 summary}

The functional difference between Conv\_1 and Conv\_0---at least on the answer-position property---is not in information content (AUC gap only 0.025) but in \textbf{specific dimensions of representational structure}. Conv\_1's output has significantly higher token-level amplitude variation (Norm Variance 3.2$\times$) and subspace dimensionality (Effective Rank 1.3$\times$), with more spatially uniform structure (decay 0.058 vs 0.109).

The divergence across metrics has diagnostic value. Conv\_0 scores slightly higher on directional distinctiveness yet is nearly dispensable ($-1.09$~F1)---ruling out directional distinctiveness as a sufficient condition. The two dimensions where Conv\_1 holds a clear advantage---amplitude modulation and subspace dimensionality---are candidate mechanisms. This is consistent with the scaled dot-product attention mechanism (Vaswani et al.~\cite{vaswani2017attention}): $QK^{\!\top}$ scores are sensitive to input magnitude differences, and higher-dimensional inputs allow $Q$, $K$ projections to carry richer discriminative signals. Notably, FFN provides a natural control: its Norm Variance (14.79) is nearly identical to Conv\_1's (14.48), so if Norm Variance alone were sufficient for importance, FFN should match Conv\_1 in ablation impact. It does not ($-3.32$ vs $-32.54$~F1), precisely because FFN sits \emph{downstream} of Self-Attention---its output properties cannot shape $QK^{\!\top}$ scores. This confirms that amplitude modulation's causal impact depends critically on pipeline position: the same output property matters most when it directly feeds into attention, and contributes far less when it does not.
...
Attention distortion under Conv\_1 removal is \textbf{8.2$\times$ greater} than under Conv\_0 removal, and this ratio is consistent across all 7 blocks (7.0--10.3$\times$).
...
Without Conv\_1, attention does not diffuse---it \textbf{collapses} into over-concentrated patterns on wrong positions (entropy \emph{decreases}, answer mass---the fraction of total attention weight falling on ground-truth answer tokens---drops 19\%). The model does not lose track of information; instead, attention becomes erroneously focused on non-answer positions.
...
Causal tracing (Exp~1c) & Conv\_1 accounts for 71.3\% of total AIE. & Confirms the ordering with a restoration-based method.\\
Linear probes (Exp~2a) & AUC gap only 0.025 on answer-position property. & Rules out information difference on this property as the primary explanation; other properties may still differ.\\
Representational analysis (Exp~2b/c) & Conv\_1: Norm Variance 3.2$\times$, Effective Rank 1.3$\times$, flatter spatial decay. & Identifies candidate mechanisms: amplitude modulation and subspace dimensionality.\\
Downstream attention (Exp~3) & JSD 8.2$\times$; entropy $-0.259$; answer mass $-19\%$. & Identifies the proximate failure mode: attention collapses onto wrong positions.\\
\bottomrule
\end{tabularx}
\end{center}
...
We choose \textbf{eval-time ablation}---zeroing sub-component outputs directly on a fully trained model (following the occlusion-based protocol described in Section~9.3.1). This is a deliberate methodological choice, not a fallback due to resource constraints. Our research goal is \textbf{mechanistic analysis}---understanding the functional roles that components \emph{currently serve} in the trained model---not architecture search. This aligns with the causal tracing methodology of Meng et al.~\cite{meng2022locating} for localising factual associations in GPT: intervening on a trained model (corruption + restoration) to localise functional roles. Training-time ablation allows the model to learn compensatory strategies during retraining, erasing the functional traces of the removed component---making it \emph{unsuitable} for mechanistic analysis.
...
The causal importance ranking is $C \odot A$ ($-15.83$) $\gg$ $C$ ($-8.51$) $>$ $A$ ($-4.04$) $\gg$ $C \odot B$ ($-1.03$). The gated term $C \odot A$ is the single most important sub-component; the Q2C direction ($C \odot B$) is nearly negligible.

At the direction level, QANet's C2Q/Q2C ratio is \textbf{61.0$\times$} (BiDAF: 2.7$\times$). This headline ratio involves unequal intervention scales (C2Q zeros 2/4 dimensions, Q2C zeros 1/4); the controlled single-component ratio is 15.4$\times$ (see Intervention size control below). Both ratios confirm that the asymmetry not only \emph{persists} after the architecture change but \emph{dramatically intensifies}. Removing C2Q drops F1 to 7.46 (approaching 7.11 from full-layer removal); Q2C ($C \odot B$) contributes only $-1.03$ F1, nearly redundant in QANet.
...
From the parameter side, $A$ (C$\to$Q main term) has the highest Frobenius norm (18.44), significantly above the other three quadrants (11.78--12.42), \textbf{supporting} C2Q dominance. However, $C \odot A$ and $C \odot B$ have similar norms (11.90 vs 11.78), failing to predict their $15.4\times$ ablation difference. Weight norms agree with ablation at the direction level ($A$ largest $\to$ C2Q dominant), but at sub-component level, input signal quality equally determines causal effect.

Notably, raw context passthrough ($C$) is the second most important sub-component ($-8.51$ F1), indicating the model relies on both attention-processed signals and unmodified raw context.

\textbf{Intervention size control.} In the direction-level comparison, C2Q ablation zeros $2/4$ dimensions ($A$ + $C \odot A$) while Q2C zeros only $1/4$ dimension ($C \odot B$)---different intervention scales. To rule out this confound, we check single-component comparisons (each zeroing exactly $1/4$ of dimensions): $C \odot A$ ($-15.83$) vs $C \odot B$ ($-1.03$) = \textbf{15.4$\times$}, $A$ ($-4.04$) vs $C \odot B$ ($-1.03$) = \textbf{3.9$\times$}. Even with strictly controlled intervention size, every C2Q sub-component significantly outweighs Q2C. The asymmetry holds at any granularity.

\ReportSubsection{10.5 Experiment 2: What mechanism maintains the asymmetry?}

Although the two directions share the same similarity matrix and differ only in softmax normalisation (Section~10.1), they perform fundamentally different operations:
...
Conditions: \textbf{ans\_only} corrupts only answer-span positions ($y_1$ to $y_2$); \textbf{non\_ans\_only} corrupts only non-answer, non-padding positions; \textbf{full\_context} corrupts all Context positions (baseline reference). The Total Effect (TE) is the drop in answer probability: $\text{TE} = P(\text{answer} \mid \text{clean}) - P(\text{answer} \mid \text{corrupted})$.
...
Under controlled single-component comparison (each zeroing exactly $1/4$ of dimensions), the core ratio is \textbf{$C \odot A$ vs $C \odot B$ = 15.4$\times$} (Section~10.4.4). Removing all C2Q sub-components drops F1 to 7.46---nearly identical to full-layer removal (7.11)---while Q2C contributes only $-1.03$~F1. \texttt{cq\_resizer} weight norms independently validate C2Q dominance at the direction level.
...
Following the eval-time intervention approach used throughout this study (Section~9.3.1), all H3 experiments are performed on a fully trained model without retraining.

Wiring replacement is a specific form of eval-time intervention: the encoder is unchanged, only the Pointer's input combinations are modified. This directly measures ``how the model currently uses each iteration product.'' Since Pointer weights are trained on specific input distributions, replacement introduces distribution mismatch---this inherent limitation is mitigated in Experiment~2 through Pointer-weight-independent linear probes.
...
\textbf{Assumption~1 (Depth specialisation)---barely holds at the behavioural level}: The asymmetric design (original) outperforms the symmetric design (sym\_M2) by only \textbf{0.23~F1}. The supposed ``specialisation'' from assigning $M_3$ to end prediction provides near-negligible marginal benefit. More notably, $M_2$ doing both tasks (sym\_M2: $-0.23$) \emph{outperforms} $M_3$ doing both tasks (sym\_M3: $-0.54$)---the more processed product is actually worse.

\textbf{Assumption~2 (Shared anchor)---near-redundant}: Removing $M_1$ (no\_M1: $-0.58$) barely affects performance, but $M_1$ alone (only\_M1: $-6.28$) is catastrophic. $M_1$ participates in both predictions yet contributes the least---the shared anchor provides negligible incremental value.

\textbf{Assumption~3 (Iterative gain)---partially holds with diminishing returns}: $M_2$/$M_3$ are essential (only\_M1: $-6.28$), but $M_2$ already captures the vast majority of incremental value. The second iteration's increment (only\_M1 vs sym\_M2: 6.04~F1) far exceeds the third iteration's marginal contribution (near zero or negative).
...
Three experiments progressively investigate the three design weaknesses observed in pre-validation:
...
The $M_2$/$M_3$ quadrant norms in $w_1$ and $w_2$ are \textbf{1.97$\times$} and \textbf{1.69$\times$} their $M_1$ counterparts respectively. Following the magnitude-based importance reasoning of Li et al.~\cite{li2017pruning}, larger norms suggest greater learned dependency. \textbf{Caveat}: in linear models, weight norms are influenced by input feature magnitudes---if $M_1$ tokens have systematically larger average magnitude than $M_2$/$M_3$, smaller weights may reflect scale compensation rather than reduced dependence. The quadrant ratio should be interpreted as \emph{directional support} (consistent with behavioural evidence), not independent strong evidence.
...
No functional specialisation is detected: $M_2$ achieves the highest AUC for \emph{both} start and end, while $M_3$ is slightly lower for both. The expected ``$M_2 \to$ start, $M_3 \to$ end'' specialisation pattern \textbf{does not appear}---all differences are within $\Delta$AUC = 0.001, well below the estimated standard error ($\sim$0.005--0.01), in the range of statistical noise. The three iteration products' boundary information content is statistically indistinguishable.
...
The ``smoothing without return'' explanation above assumes the third iteration's transformation is functionally redundant. A competing explanation exists: during training, stochastic depth (Huang et al.~\cite{huang2016stochastic}) skips sub-layers with probability $p_{\text{drop}} \times l/L$ (deeper layers have higher skip probability). Layers 15--21 (traversed only by $M_3$) have the highest skip probability, which may cause $M_3$'s representations to be \textbf{insufficiently optimised} rather than ``over-smoothed''---since encoder parameters are weight-shared across all three iterations, the shared weights receive weaker gradient signal from the third pass, biasing them toward the input distributions of earlier iterations. If this explanation holds, the $M_2 \to M_3$ structural changes reflect a training regularisation side-effect, not an inherent deficiency of weight-shared iteration. Current experiments cannot distinguish the two hypotheses, as both predict $M_3$ representation quality below $M_2$. We retain both explanations in parallel.
...
\textbf{1.\ Depth specialisation---not detected.} Pre-validation observes a behavioural advantage of only 0.23~F1 for the asymmetric design; the following information-level analysis explains why. Linear probes show $M_2$ achieves the highest AUC for both start (0.979) and end (0.981), with $M_3$ slightly lower for both (start = 0.978, end = 0.980). The expected ``$M_2 \to$ start, $M_3 \to$ end'' specialisation pattern \textbf{does not appear}---all differences are within $\Delta$AUC = 0.001, well below the estimated standard error ($\sim$0.005--0.01), in the range of statistical noise. Wiring replacement (behavioural) and probes (informational) consistently point to $M_2 \geq M_3$; the 0.23~F1 marginal advantage more likely reflects Pointer weight adaptation to training distributions than genuine functional specialisation.

\textbf{2.\ Shared anchor---near-redundant.} Replacing $M_1$ causes only $-0.58$~F1 (behavioural). Pointer weights' $M_2$/$M_3$ quadrant norms are 1.7--2.0$\times$ $M_1$ quadrants (parameter level; $w_1$: 1.97$\times$, $w_2$: 1.69$\times$). CKA($M_1$, $M_2$) = 0.871 shows $M_2$ and $M_1$ share substantial linear structure (information level). All three angles point in the same direction: \textbf{$M_1$ provides negligible incremental value in the $[M_1;\, M_2]$ concatenation}. Note that no\_M1 substitutes $M_2$/$M_3$ into $M_1$'s position (not pure zeroing); the $-0.58$ loss partly reflects representational similarity.

\textbf{3.\ Iterative gain---second iteration captures the vast majority of value; third iteration's marginal contribution is near zero.} The only\_M1 vs sym\_M2 gap of 6.04~F1 shows the second iteration is essential; but the third iteration's marginal F1 contribution is negligible. Experiment~3 observes a noteworthy phenomenon: the third iteration further smooths representation structure (local contrast $-28\%$, boundary sharpness $-25\%$), yet linearly readable boundary information barely changes ($\Delta$AUC = 0.001). Information persists in a more distributed encoding---\textbf{structural change does not equal information loss}. However, $M_3$'s structural changes may partly stem from stochastic depth biasing shared weights toward earlier iterations (see Experiment~3 limitations), not solely from inherent effects of iteration.
...
Third, \textbf{stochastic depth training-time effects}: As discussed in Experiment~3's main text, $M_3$'s structural changes may partly stem from stochastic depth biasing shared weights toward earlier iterations rather than iterative over-smoothing. Current experiments cannot distinguish the two. This is the primary alternative explanation for Conclusion~3 (iterative gain exhaustion).
...
\ReportSection{12 Concluding Discussion}

This report pursued two objectives: systematically repairing a deliberately flawed QANet implementation (Sections~1--7) and conducting hypothesis-driven mechanistic analyses on the resulting model (Sections~9--11). We conclude by synthesising the insights gained from both parts.

\ReportSubsection{12.1 Debugging: from executability to correctness}

The repair process addressed 56 issues spanning every module of the pipeline---from input embedding and convolution operators through attention mechanisms, output layers, optimisers, schedulers, and training infrastructure. The progression from Stage~I (pipeline executability) to Stage~II (mechanism correctness) revealed a recurring pattern: many errors did not produce runtime failures but silently degraded training dynamics. Incorrect depthwise-separable convolution order, inverted dropout scaling, and flawed bias-correction exponents are representative examples---each is numerically permissible yet distorts the optimisation trajectory. Ensuring that the repaired model trains stably under multiple optimiser families (SGD, SGD-momentum, Adam; Section~8) provided confidence that the codebase is a legitimate experimental platform rather than a fragile special case.

\ReportSubsection{12.2 A unifying pattern across three levels}

Three hypotheses examined QANet at sub-layer (H1), sub-component (H2), and macro (H3) scope. Despite targeting different modules with different methods, a single pattern recurs: \textbf{coexisting critical bottlenecks and near-redundancies}. Conv\_1 is indispensable ($-32.54$~F1) while Conv\_0 is compensable ($-1.09$~F1); C2Q is irreplaceable ($-62.78$~F1) while Q2C is redundant ($-1.03$~F1); $M_2$ captures the vast majority of iterative gain (6.04~F1) while $M_1$ and $M_3$ contribute marginally. At every level, the trained model concentrates its functional dependence on a narrow subset of its architectural capacity.

This convergence is not trivially expected. H1 measures importance via ablation and causal tracing; H2 uses sub-component zeroing and selective corruption; H3 uses wiring replacement and linear probes. That four independent evidence types---behavioural (ablation, wiring replacement), informational (linear probes, CKA), parametric (weight norms), and structural (representational metrics)---all point in the same direction strengthens the conclusion beyond what any single experiment could establish.

A mechanistic thread also connects the three: H1 found that Conv\_1 builds high-discriminability local features; H3 found that iterative Self-Attention progressively consumes them ($M_1 \to M_2 \to M_3$ local contrast drops from 0.116 to 0.048). Meanwhile, H2 showed that the sole cross-stream information pathway (C2Q) is the critical bottleneck \emph{before} the Model Encoder even begins. Together, they trace the flow of task-relevant information from injection (C2Q) through local feature construction (Conv\_1) to iterative refinement (encoder passes)---and identify where diminishing returns set in.

\ReportSubsection{12.3 Connecting debugging and mechanism analysis}

The debugging and mechanism phases are independently motivated but mutually reinforcing. The repair process ensured that every component operates as theoretically intended, so that mechanistic observations can be attributed to the model's learned behaviour rather than to implementation artifacts. Conversely, the mechanism study provided indirect validation of the repair: Conv\_1's importance is reproducible across two independent methods (ablation and causal tracing); the C2Q/Q2C ratio at the qualitative level is consistent with BiDAF's published retraining results despite our use of a different (eval-time) methodology; representational structure metrics show interpretable monotonic patterns across encoder iterations. An incorrectly repaired model would be unlikely to produce such internally consistent and externally corroborated results.

\ReportSubsection{12.4 Limitations and future work}

All mechanistic experiments were conducted at eval time on a single trained checkpoint. While this is appropriate for mechanistic analysis (understanding what the model \emph{currently does}), it cannot distinguish learned behaviour from training-time confounds such as stochastic depth effects (H3) or compensatory adaptation. Retraining-based interventions---varying encoder depth, disabling specific sub-layers during training, or training without weight sharing---would enable causal claims that our current eval-time methodology cannot support. Additionally, the model was trained on a subset of SQuAD~1.1 under constrained compute; whether the observed patterns generalise to full-scale training or other QA datasets remains an open question.
...
This appendix lists the hyperparameters of the trained QANet model used across all mechanistic experiments (Sections~9--11). The model was trained on a subset of SQuAD~1.1 (Rajpurkar et al.~\cite{rajpurkar2016squad}) using the Adam optimizer with inverse-log warmup, following the recipe described in Section~8. All mechanism experiments are performed at eval time on the resulting checkpoint; no hyperparameters were modified between hypothesis sections.

\vspace{0.6em}

{\bfseries Table A.1. Model architecture and training configuration}

\vspace{0.3em}

\begin{center}
\begin{tabularx}{0.96\linewidth}{>{\raggedright\arraybackslash}p{0.30\linewidth}X}
\toprule
\textbf{Parameter} & \textbf{Value} \\
\midrule
\multicolumn{2}{l}{\textit{Architecture}} \\[0.2em]
Hidden dimension ($d_{\text{model}}$) & 96 \\
Attention heads & 8 \\
Embedding Encoder & 1 block, 4 convolutions ($k=7$), 1 self-attention, 1 FFN \\
Model Encoder & 7 blocks $\times$ 3 passes ($\to M_1, M_2, M_3$); 2 convolutions ($k=5$, depthwise separable), 1 self-attention, 1 FFN per block \\
\midrule
\multicolumn{2}{l}{\textit{Training}} \\[0.2em]
Optimizer & Adam ($\beta_1=0.8$, $\beta_2=0.999$, $\varepsilon=10^{-7}$) \\
Learning rate & $1 \times 10^{-3}$ with inverse-log warmup over 1000 steps \\
Batch size & 32 \\
Training budget & 60{,}000 steps; early stopping (patience 50 checkpoints $\times$ 200 steps) \\
Selected checkpoint & Step 40{,}800 (F1\,=\,70.24, EM\,=\,59.22) \\
\midrule
\multicolumn{2}{l}{\textit{Regularisation}} \\[0.2em]
Dropout & 0.3 \\
Dropout (char embedding) & 0.15 \\
Stochastic depth & Coupled with dropout: $p_{\text{drop}}(l) = \text{dropout} \times l/L$; last-layer survival $p_L = 0.7$ \\
EMA decay & 0.9999 \\
\midrule
\multicolumn{2}{l}{\textit{Mechanism experiments (Sections~9--11)}} \\[0.2em]
Evaluation set & SQuAD 1.1 subset dev split (1{,}309 examples) \\
Causal tracing samples & 200 (H1, H2); 50 (attention analysis, H1 Exp~3) \\
Noise scale ($\sigma$) & $3 \times \text{std}(\text{clean activations})$ \\
Linear probe samples & 500 (80/20 train/test split) \\
Representation analysis samples & 200 (H1 Exp~2, H3 Exp~3) \\
\bottomrule
\end{tabularx}
\end{center}
...
\ReportSection{Appendix C\quad Implementation Stack}

The following references were provided in the assignment template but were not directly cited in the main text. They correspond to the foundational libraries and techniques on which the QANet codebase is built. We include them here to acknowledge their role in the implementation.
...
Layer normalisation      & Ba et al.~\cite{ba2016layernorm} \\
Batch normalisation      & Ioffe \& Szegedy~\cite{ioffe2015batchnorm} \\
Dropout                  & Srivastava et al.~\cite{srivastava2014dropout} \\
ReLU activation          & Glorot et al.~\cite{glorot2011relu} \\
\bottomrule
\end{tabularx}
